...
const offerPrice = computed(() =>{
    if (props.product.pivot.discount_type === 'flat'){
        return  props.product.price - props.product.pivot.discount;
    }else{
        let discount = (props.product.price  * props.product.pivot.discount) / 100;
...
const addToCart = () => store.addToCart({...props.product, price:offerPrice, quantity:1})
...
<template>
    <div class="card offer">
        <div class="card-body border border-primary relative" @click="showProductDetails">
            <span class="discount_banner">{{ props.product?.pivot.discount_type === "flat" ? $showPrice(props.product?.pivot.discount) : props.product?.pivot.discount+"%"}} OFF</span>
            <img class="card-img-top" :src="props.product.thumbnail" alt="">
            <div class="card-product-details card-text d-flex flex-column text-center">
                <p class="p-0 m-0">{{ props.product?.title?.slice(0, 30) }}</p>
                <p class="p-0 m-0">{{ props.product.unit }}</p>

                <div class="d-flex justify-content-between">
                    <h6>{{ $showPrice(offerPrice) }}</h6>
                    <s class="fs-5">{{ $showPrice(product.price) }}</s>
                </div>
            </div>
        </div>
            ...
            <div class="col-12 col-md-7">
                ...
                <div class="ecommerce-details-price d-flex flex-wrap mt-1 align-items-center justify-content-between">
                    <div class="d-flex align-items-center gap-2">
                        <h2>{{ $showPrice(offerPrice) }}</h2>
                        <s class="fs-5">{{ $showPrice(product.price) }}</s>
                    </div>
                    <div>
                        <div class="p-1 bg-primary text-white discount-price-show" v-if="product.pivot.discount">{{product.pivot.discount_type === 'flat' ? $showPrice(product.pivot.discount) : product.pivot.discount+" %" }} Off</div>
                    </div>
                </div>
